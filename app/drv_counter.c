...
void delay_us_refclk(uint32_t nticks) {
    uint64_t refclk_cntr_value = refclk_cntr_val64();
    nticks = (float) get_SOC_REFCLK() * nticks / 1000000.0f;
    refclk_cntr_value += nticks;
    while (refclk_cntr_val64() < refclk_cntr_value);
}
